...
func (n *noteRepository) Update(ctx context.Context, note *entity.Note) error {
	_, err := n.db.Exec(
		ctx,
		`UPDATE notes SET title = $1, content = $2, notebook_id = $3, updated_at = $4 WHERE id = $5`,
		note.Title,
		note.Content,
		note.NotebookId,
		note.UpdatedAt,
...
func (n *noteRepository) DeleteByNotebookId(ctx context.Context, notebookId uuid.UUID) error {
	_, err := n.db.Exec(
		ctx,
		`UPDATE notes SET is_deleted = true, deleted_at = $1 WHERE notebook_id = $2`,
		time.Now(),
		notebookId,
	)
	if err != nil {
...
func (n *noteRepository) GetByIds(ctx context.Context, ids []uuid.UUID) ([]*entity.Note, error) {
	...
	rows, err := n.db.Query(
		ctx,
		fmt.Sprintf(`SELECT id, title, content, notebook_id, created_at, updated_at FROM notes WHERE id IN (%s) AND is_deleted = false`, idStrJoined),
	)
	if err != nil {
		return nil, err
	}
