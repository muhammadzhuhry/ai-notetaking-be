...
func (cs *consumerService) processMessage(ctx context.Context, msg *message.Message) {
	...
	// Process embedding
	res, err := embedding.GetGeminiEmbedding(os.Getenv("GOOGLE_GEMINI_API_KEY"), content, "RETRIEVAL_DOCUMENT")
	if err != nil {
		panic(err)
	}
	...
	err = noteEmbeddingRepo.DeleteByNoteId(ctx, note.Id)
	if err != nil {
		panic(err)
	}

	err = noteEmbeddingRepo.Create(ctx, entity)
	if err != nil {
		panic(err)
	}
